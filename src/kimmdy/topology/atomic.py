...
class Atom:
    ...
    def eq_by_params(self, other):
        """Compare two atoms by thair parameters, ignoring bound_to_nrs and is_radical."""
        if not isinstance(other, Atom):
            logger.warning(
                f"Comparing Atoms with different types: {type(self)} and {type(other)}. Returning False."
            )
            return False
