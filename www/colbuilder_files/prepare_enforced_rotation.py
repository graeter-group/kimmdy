...
def prepare(file_dict: dict):
    ...
    enforced_rotation = {'rotation':'Yes','rot-nstrout':'1000','rot-ngroups':len(rotation_groups),'rot-type':'rm-pf','rot-vec':'0 0 1','rot-rate':'0.0','rot-k':'200','rot-fit-method':'norm'}
    mdp_append = []
    mdp_append.append("\n\n; Enforced rotation general settings")
    mdp_append.append(f"rotation                 = {enforced_rotation['rotation']}")
    mdp_append.append(f"rot-nstrout              = {enforced_rotation['rot-nstrout']}           ; Output frequency")
    mdp_append.append(f"rot-ngroups              = {enforced_rotation['rot-ngroups']}           ; Number of rotation groups")
    mdp_append.append(f"\n; Per group settings")
    mdp_append.append(f"; rot-group: Rotation group name")
    mdp_append.append(f"; rot-type: Rotation potential. Can be iso, iso-pf, pm, pm-pf, rm, rm-pf, rm2, rm2-pf, flex, flex-t, flex2, flex2-t")
    ...
    mdp_append.append(f"; rot-fit-method: Fitting method to determine angle of rotation group (rmsd, norm, or potential)")
    for i,group in enumerate(rotation_groups):
        mdp_append.append(f"rot-group{i}              = {group}")
        mdp_append.append(f"rot-type{i}               = {enforced_rotation['rot-type']}")
        mdp_append.append(f"rot-vec{i}                = {enforced_rotation['rot-vec']}")
        mdp_append.append(f"rot-rate{i}               = {enforced_rotation['rot-rate']}")
        mdp_append.append(f"rot-k{i}                  = {enforced_rotation['rot-k']}")
        mdp_append.append(f"rot-fit-method{i}         = {enforced_rotation['rot-fit-method']}")
