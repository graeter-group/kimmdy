...
class ChangeManager:
    ...
    def modify_top(self, oldtop, newtop):
        ...
        for i in range(len(data_all)):
            if self.breakpair[0] in data_array[i][0:4] and self.breakpair[1] in data_array[i][0:4]:    #[0:4] since value "func" should not be considered (leads e.g. to problem if '1' in breakpair)
        ...
        for j in range(len(list_of_dihedrals)):
            if (self.breakpair[0] in list_of_dihedrals[j][0:4]) and (self.breakpair[1] in list_of_dihedrals[j][0:4]):
