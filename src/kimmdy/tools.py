...
def create_plumed_dat(
    top_path: Path,
    idx_path: Path,
    idx_group: Optional[str],
    entries_to_remove: list = ["C-N", "H*", "O*"],
    stride: int = 100,
):
    print(
    ...
    if idx_group is None:
        print("Which index group should be written to the plumed input?")
        for g in idx.keys():
            print(g)
        print("")
        while idx_group is None:
            _idx_group = input("Please enter the index group:")
            if _idx_group in idx.keys():
                idx_group = _idx_group
            else:
                print(f"Error: {_idx_group} not in index file.")
...
def entry_point_create_plumed():
    parser = argparse.ArgumentParser(
        description="Build plumed input file. Requires a topology file and a gromacs index file."
    )
    parser.add_argument(
        "--entries-to-remove",
        nargs="*",
