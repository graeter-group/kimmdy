...
class Homolysis(Reaction):
    ...
    def rupturerates(self,plumedfile,datafile,topfile,filepath_bonds,filepath_edis):
        ...
        for j in range(len(list_of_breakpairs_and_distances)):
            ...
            r_0, k_f = self.find_bond_param(atomtypes, filepath_bonds) #read out from gromacs force field
            E_dis = self.find_Edis(atomtypes, filepath_edis)  #read out from gromacs force field
            ...
            list_of_recipes.append(ConversionRecipe(ConversionType.BREAK, breakpair,atomtypes))
