...
def find_bond_param(atomtypes, filepath):
    ...
    # only use first entry correspondnig to [ bondtypes ] section
    found_first_entry = False
    for i in range(len(data_array)):
        # or data_array[i][:2] == (atomtype2, atomtype1)) :
        if (list(data_array[i][:2]) == comb1 or list(data_array[i][:2]) == comb2):
            r_0 = data_array[i][3]
            k_f = data_array[i][4]
...
def find_Edis(atomnames, filepath):
    ...
    if not Edis:
        ...
        logging.info(
            "Used simplified atom types to determine dissociaton energy for morse potential leading to E_dis = " + str(Edis))
    ...
    if not Edis:
        ...
        logging.warning(
            "No morse dissociation energy found. Used 350 as default value. Atoms were: " + str(atomnames))
...
def find_distances(plumedfile, datafile):
    ...
    with open(datafile) as f:
        for line in f:
            ...
            for k in range(1, nbr_of_pairs + 1):
                distance = float(line_split[k])
                distances[k-1][ctr] = distance
            ctr += 1
    nbr_of_data_points = ctr

    print('Collected distances from ' + str(datafile) + ' for ' + str(nbr_of_pairs) +
          ' pairs with ' + str(nbr_of_data_points) + ' distances per pair.')
    logging.info('Collected distances from ' + str(datafile) + ' for ' + str(nbr_of_pairs) +
                 ' pairs with ' + str(nbr_of_data_points) + ' distances per pair.')
...
def force_correction_factor(aminoacid, atomnames):
    ...
    acids_without_correction = ['L5Y', 'L4Y', 'DOP', 'ACE', 'NME']
    ...
    if 'N' in atomnames and aminoacid not in acids_without_correction:
        baseline = float(df_forces.force.loc[(df_forces.aminoacid == aminoacid) & (
            df_forces.bondtype == 'CA-N').tolist()])
    elif 'C' in atomnames and aminoacid not in acids_without_correction:
        baseline = float(df_forces.force.loc[(df_forces.aminoacid == aminoacid) & (
            df_forces.bondtype == 'CA-C').tolist()])
    elif 'N' in atomnames and aminoacid == 'DOP':  # Dopas are made of TYR or PHE, average as proxy
        baseline1 = float(df_forces.force.loc[(df_forces.aminoacid == 'PHE') & (
            df_forces.bondtype == 'CA-N').tolist()])
        baseline2 = float(df_forces.force.loc[(df_forces.aminoacid == 'TYR') & (
            df_forces.bondtype == 'CA-N').tolist()])
    ...
    # backbone bond in crosslink residues
        ...
        baseline = float(df_forces.force.loc[(df_forces.aminoacid == 'LYS') & (
            df_forces.bondtype == 'CA-N').tolist()])
    ...
    # crosslink bonds
        ...
        baseline = float(df_cross.force.loc[(df_cross.aminoacid == aminoacid) & (
            df_cross.bondtype == bondtype).tolist()])
...
def calc_transition_rate(r_curr, r_0, E_dis, k_f, aminoacid, atomnames):
    ...
    # pre-exponential factor #from fitting averaged C_a - N data to gromacs data, see KIMMDY paper  #or: 1/2pi sqrt(k/m)
    k_0 = 0.288
    ...
    if r_curr > r_infl:
        logging.info(
            'Used maximum force for bond Evans model since position behind inflection point found')
        F = 2*beta*E_dis*np.exp(-beta*(r_infl-r_0)) * \
            (1-np.exp(-beta*(r_infl-r_0)))
    else:
        F = 2*beta*E_dis*np.exp(-beta*(r_curr-r_0)) * \
            (1-np.exp(-beta*(r_curr-r_0)))

    delta = force_correction_factor(aminoacid, atomnames)
    F = F + delta
    logging.debug('Average bond elongation of ' + str(r_curr-r_0) +
                  ' nm leads to calculated force in bond F = ' + str(F) + ' after applying correction of ' + str(delta))

    # calculate extrema of shifted potential i.o.t. get barrier hight
    rmin = r_0 - 1/beta * \
        np.log((beta * E_dis + np.sqrt(beta**2 * E_dis **
               2 - 2*E_dis*beta*F))/(2*beta*E_dis))
    rmax = r_0 - 1/beta * \
        np.log((beta * E_dis - np.sqrt(beta**2 * E_dis **
               2 - 2*E_dis*beta*F))/(2*beta*E_dis))

    # Note: F*r should lead to same result as F*(r-r_0) since the shifts in Vmax-Vmin adds up to zero
    Vmax = E_dis*(1-np.exp(-beta*(rmax-r_0)))**2 - F * (rmax - r_0)
    ...
    if F <= 0.0:  # negative force: Vmax -> infinity impliying k -> 0
        k = 0.0
        logging.debug(
            'Found negative force, most likely due to compression. Rate replaced with zero.')
...
def calc_av_rate(distances, aminoacid, atomtypes, atomnames, filepath_bonds, filepath_edis):
    ...
    r_0, k_f = find_bond_param(atomtypes, filepath_bonds)
    ...
    if len(distances) > 1:
        ...
        print(r_av)
    k, F = calc_transition_rate(r_av, float(r_0), float(
        E_dis), float(k_f), aminoacid, atomnames)
    #print(r_av, k,F)
    #print("Average distance first, calculate force leads to force: " +str(F))
    logging.info(
        "Average distance first, calculate force leads to force: " + str(F))
...
def calc_rate_afterwards_av(distances, r_0, E_dis, k_f):
    ...
    logging.info("Calculating forces, average results lead to: " +
                 str(np.mean(F_all)))
...
def delete_bonded_interactions(oldtop, newtop, breakpair):
    ...
    logging.info('Info: Start deleting bond ' +
                 str(breakpair) + ' in Topology')
    ...
    logging.debug(
        'Deleted bonds, angles and dihedrals. Now starting deleting pairs: ')
    pairs_to_be_deleted = []
    for j in range(len(list_of_dihedrals)):
        if (breakpair[0] in list_of_dihedrals[j][0:4]) and (breakpair[1] in list_of_dihedrals[j][0:4]):
            # pairs are sorted
            if float(list_of_dihedrals[j][0]) < float(list_of_dihedrals[j][3]):
    ...
    for k in range(len(list_of_pairs)):
        if list_of_pairs[k] in pairs_to_be_deleted:
            ...
            logging.debug('Deleted in .top:' +
                          data_new[k + pairs - deleted_pairs + 1])
...
def modify_radical_interactions(oldtop, newtop, old_bond, reference_bond, old_to_new):
    ...
    # save pairs, dihedrals and their respective positions in new data set for next step
    for i in range(len(data_all)):
        # modify the ones with old breakpair #[0:4] since value "func" should not be considered (leads e.g. to problem if '1' in breakpair)
        if reference_bond[0] in data_array[i][0:4] and reference_bond[1] in data_array[i][0:4]:
            logging.debug('Copied in .top: ' + data_all[i])
            new_interaction_to_be_inserted = data_all[i]
            new_interaction_to_be_inserted = new_interaction_to_be_inserted.replace(
                old_to_new[0], old_to_new[1], 1)  # replace (first) occurance of old bond with new bonding partner
    ...
    logging.debug(
        'Added bonds, angles and dihedrals. Now start finding pairs: ')
    ...
    for k in range(len(list_of_pairs)):
        if list_of_pairs[k] in pairs_to_be_modified:
            if old_to_new[0] in list_of_pairs[k]:
                ...
                # replace (first) occurance of old bond with new bonding partner, if in there
                new_interaction_to_be_inserted = old_interaction.replace(
                    old_to_new[0], old_to_new[1], 1)
                print('will modify pair ' + old_interaction +
                      'to ' + new_interaction_to_be_inserted)
                logging.debug('will modify pair ' + old_interaction +
                              'to ' + new_interaction_to_be_inserted)
                data_new = np.insert(
                    data_new, k + pairs - deleted_pairs + 2,  new_interaction_to_be_inserted)
                data_new = np.delete(
                    data_new, k + pairs - deleted_pairs + 3)  # delete old pair
            else:  # delete other pairs
                logging.debug('will delete pair ' +
                              data_new[k + pairs - deleted_pairs + 2])
                print('will delete pair ' +
                      data_new[k + pairs - deleted_pairs + 2])
...
def modify_hbond(referencetop, oldtop, newtop, old_bond, reference_bond, old_to_new):
    ...
    # original topology that includes the new h-bond but not the old one anymore
    reference_all, reference_array = get_data_from_file(intermediatetop)
    reference_bond_str = str(reference_bond)
    newbond = reference_bond_str.replace(old_to_new[0], old_to_new[1], 1)
    ...
    for i in range(len(reference_all)):
        # modify the ones with old breakpair #[0:4] since value "func" should not be considered (leads e.g. to problem if '1' in breakpair)
        if reference_bond[0] in reference_array[i][0:4] and reference_bond[1] in reference_array[i][0:4]:
            ...
            if reached_angles == False:
                ...
                logging.debug(
                    ' add  bond' + str(new_interaction_to_be_inserted))
                data_new = np.insert(data_new, bonds + 2,
                                     new_interaction_to_be_inserted)
                inserted = inserted + 1
            elif reached_dihedrals == False:
                #print (' add  angle' + str(new_interaction_to_be_inserted))
                logging.debug(' add  angle' +
                              str(new_interaction_to_be_inserted))
                data_new = np.insert(
                    data_new, angles + 2 + inserted, new_interaction_to_be_inserted)
                inserted = inserted + 1
            elif reached_dihedrals == True and proper_set == True:
                data_new = np.insert(
                    data_new, dihedrals + 2 + inserted, new_interaction_to_be_inserted)
                print(' add  dihedral' + str(new_interaction_to_be_inserted))
                logging.debug(' add  dihedral' +
                              str(new_interaction_to_be_inserted))
                inserted = inserted + 1
            elif reached_dihedrals == True and proper_set == False:
                data_new = np.insert(
                    data_new, improper + 2 + inserted, new_interaction_to_be_inserted)
                #print (' add  improper' + str(new_interaction_to_be_inserted))
                logging.debug(' add  improper' +
                              str(new_interaction_to_be_inserted))
                inserted = inserted + 1
    ...
    for pair in pairs_to_be_modified:
        ...
        if old_to_new[0] in pair:
            # replace (first) occurance of old bond with new bonding partner, if in there
            new_interaction_to_be_inserted = pair_str.replace(
                old_to_new[0], old_to_new[1], 1)
            ...
            logging.debug('add pair ' + new_interaction_to_be_inserted +
                          ' which was formerly ' + str(pair))
            data_new = np.insert(data_new, pairs + inserted + 3,
                                 new_interaction_to_be_inserted)
            inserted = inserted + 1
        else:
            pair_str = pair_str + '     1'
            logging.warning(
                'Warning: will add pair that might have not been there before: ' + pair_str)
            print(
                'Warning: will add pair that might have not been there before: ' + pair_str)
...
def modify_atomtype(topology, reference_topology, old_to_new):
    ...
    for j in range(len(reference_all)):
        ...
        if atoms and reference_atom in reference_array[j][0]:
            ...
            print(' will change atomtype of ' + str(atom) + ' to type ' +
                  str(reference_type) + ' with partial charge ' + str(reference_charge))
            logging.debug(' will change atomtype of ' + str(atom) + ' to type ' +
                          str(reference_type) + ' with partial charge ' + str(reference_charge))
            break
...
def modify_plumedfile(plumedfile, plumedfile_new, distancefile_new, ruptured_bonds):
    ...
    with open(plumedfile) as f:
        for line in f:
            ...
            file.write(line)
    file.close()
    print('Adjusted Plumedfile: Commented out distances ' + str(broken_distnbr) + ' corresponding to breakpairs: ' +
          str(rupture_pairs) + '. \n Will now write distances to: ' + str(distancefile_new))
    logging.info('Adjusted Plumedfile: Commented out distances ' + str(broken_distnbr) + ' corresponding to breakpairs: ' +
                 str(rupture_pairs) + '. \n Will now write distances to: ' + str(distancefile_new))
...
def find_candidates(grofile, trrfile, cutoff_distance, reaction_time, radical):
    topology = grofile
    trajectory = trrfile
    print('Using ' + str(grofile) + ' and ' +
          str(trrfile) + ' to find a reaction candidate')
    logging.info('Using ' + str(grofile) + ' and ' +
                 str(trrfile) + ' to find a reaction candidate')
    distance = cutoff_distance
    u = MDAnalysis.Universe(topology, trajectory)
    # ignore solvent etc. problem: Unusual residues such as DOPA won't work with selection 'protein'
    protein = u.select_atoms('not resname SOL')
    radical_atom = protein.select_atoms('bynum ' + str(radical))[0]
    print(protein, 'protein atoms with frames in trajectory = ' +
          str(len(u.trajectory)))
    logging.debug(
        protein, 'protein atoms with frames in trajectory = ' + str(len(u.trajectory)))
    candidates = np.array([])
    time = np.array([])
    frame_ctr = 0
    ...
    # at t = 0
    nearR = protein.select_atoms(
        'around ' + str(distance) + ' bynum ' + str(radical))
    H_nearR = nearR.select_atoms('type H')
    print('H-Atoms nearby ' + str(radical) + ' (max. distance = ' +
          str(distance) + ' Angstroem) at point of breakage: ')
    for atom in H_nearR:
        print(atom)

    curr = protein.select_atoms(
        'around ' + str(distance) + ' bynum '+str(radical), updating=True)
    ...
    for atom in H_nearR:
        ...
        for ts in u.trajectory:
            ...
            radical_pos = np.split(radical_atom.position, 3)
            dist = np.sqrt((radical_pos[0] - atom_pos[0])**2 + (
                radical_pos[1] - atom_pos[1])**2 + (radical_pos[2] - atom_pos[2])**2)
    ...
    # +1 since index and atom number is shifted
    min_candidate = str(dist_dic[min_dist].index + 1)
    ...
    print(' Candidate with smallest average distance ' +
          str(min_dist) + ' = ' + str(min_candidate))
    logging.info(' Candidate with smallest average distance ' +
                 str(min_dist) + ' = ' + str(min_candidate))
...
def do_kinetic_mc(list_of_nbrs_and_atomtypes, list_of_rates):
    ...
    print('Rate = ' + str(rate) + ', breakpair = ' + str(breakpair) + ', atomtypes = ' +
          str(atomtypes) + 'jump [ps] = ' + str(delta_t) + 'with total Rate = ' + str(total_rate))
    logging.info('Rate = ' + str(rate) + ', breakpair = ' + str(breakpair) + ', atomtypes = ' +
                 str(atomtypes) + 'jump [ps] = ' + str(delta_t) + 'with total Rate = ' + str(total_rate))
...
def do_rejection_KMC(list_of_breakpairs, k_0, topfile, filepath_bonds, filepath_edis):
    ...
    random.seed()
    # choose index randomly. Note: Indices go from 0 to length-1 since computer people think differently
    t = random.randint(0, nbr_of_breakpairs - 1)
    ...
    r_0, k_f = find_bond_param(atomtypes, filepath_bonds)
    # read out from gromacs force field
    E_dis = find_Edis(atomtypes, filepath_edis)
    k = calc_av_rate(distances, float(r_0), float(E_dis), float(k_f))
    print('Info: For ' + str(breakpair) + str(atomtypes) + ' as candidate in r-kMC calculated rupture rate using ' +
          str((len(distances))) + ' distances per bond: ' + str(k) + ' per ps.')
    logging.info('For ' + str(breakpair) + str(atomtypes) + ' as candidate in r-kMC calculated rupture rate using ' +
                 str((len(distances))) + ' distances per bond: ' + str(k) + ' per ps.')
...
def check_if_error_occured(filepath):
    ...
    for i in range(len(data_array)):
        if 'error' in data_all[i]:
            error = True
            print("Warning: Simulation seems to have stopped due to an error: " +
                  str(data_all[i:i+4]))
            logging.warning(
                "Simulation seems to have stopped due to an error:" + str(data_all[i:i+4]))
...
def write_conditions_in_plumedfile(topfile, indexfile, indexgroup, parameterfile):
    ...
    for j in range(bonds_pos+2, len(data_all)):  # go through all bonds
        ...
        elif "H" in dic_of_atoms[nbr1] or "H" in dic_of_atoms[nbr2]:
            pass
        # leave out oxygen bonds
        elif "O" in dic_of_atoms[nbr1] or "O" in dic_of_atoms[nbr2]:
            pass
        else:
            bond = (nbr1, nbr2)
    ...
    for pair_nbr in range(cond_nconds):
        ...
        file.write('d' + str(pair_nbr) + ': DISTANCE ATOMS=' +
                   str(nbr1) + ',' + str(nbr2) + ' \n')
        print_arg += 'd' + str(pair_nbr) + ','
    file.write(' \n#Print distances ARG to FILE every STRIDE steps \n')
    file.write('PRINT ARG=' + str(print_arg) +
               ' STRIDE=100 FILE=distances.dat')
    file.close()
