...
def get_data_from_file(filepath):
    ...
    return data_all, data_array  # , settings
...
def find_bond_param(atomtypes, filepath, own_r0=False):
    ...
    # use own equilibrium distance r0 from run without force since they differ from the r0 in the .itp - file if bonds are not constrained.
    if own_r0:
        # print('Used own r0 from run without force instead of value from itp-file as equilibrium distance.')
        logging.debug(
            "Used own r0 from run without force instead of value from itp-file as equilibrium distance."
        )
        comb1_equil = (atomtypes[0], atomtypes[1])
        comb2_equil = (atomtypes[1], atomtypes[0])
        dic_of_equil_r0 = {
            ("N3", "CT"): "0.1476",
            ("CT", "C"): "0.1535",
            ("C", "N"): "0.1341",
        ...
        # Note CT-C seems to differ if 5-11 or 15-21
        if comb2_equil in dic_of_equil_r0:
            ...
            print("Tried to use own r0 but found no entry for " + str(atomtypes))
            logging.info("Tried to use own r0 but found no entry for " + str(atomtypes))
...
def find_Edis(atomtypes, filepath):
    ...
    if not Edis:
        ...
        logging.debug(
            "Used simplified atom types to determine dissociaton energy for morse potential"
        )
    ...
    if not Edis:
        print("Warning: No morse dissociation energy found. Used 300 as default value")
        logging.warning("No morse dissociation energy found. Used 300 as default value")
        Edis = 300  # default value in same order of magnitude
    return Edis
...
def find_distances(plumedfile, datafile):
    ...
    print(
        "Collected distances from "
        + str(datafile)
        + " for "
        + str(nbr_of_pairs)
        + " pairs with "
        + str(nbr_of_data_points)
        + " distances per pair."
    )
    logging.info(
        "Collected distances from "
...
def calc_transition_rate(r_curr, r_0, E_dis, k_f):
    ...
    beta = np.sqrt(k_f / (2 * E_dis))  # beta = 21.5 #1/nm since beta = sqrt(k/2D)
    ...
    if r_curr > r_infl:
        # print ("Info: Used maximum force for bond Evans model since position behind inflection point found")
        logging.debug(
    ...
    # print ('Info: Calculated force in bond F = ' + str(F))
    ...
    # Vcurr = E_dis*(1-np.exp(-beta*(r_curr-r_0)))**2 - F * r_curr

    delta_V = Vmax - Vmin  # -Vcurr. Choose Vmin or Vcurr as reference point?
    kT = 2.479  # gromacs units kj *mol^-1
    # tau =  0.16    #h/kT = 0.16 ps from transition state theory #pre-exponential factor
    k_0 = 0.324  # 0.324 from fitting averaged data to gromacs data #2.87 #0.0023 #calculated by comparison of rates CT-N  #2.87 #calculated from fit of Dialanin rupture rates #Or: 1/tau from transistion state, roughly 6*10**-12 #Or: 1/2pi sqrt(k/m)
    ...
    if F <= 0.0:  # negative force: Vmax -> infinity impliying k -> 0
        ...
        # print('Found negative force, most likely due to compression. Rate replaced with zero.')

    # convert rate into probability for given time steps*dt: #p = 1.0 - np.exp(-k*n*dt)

    return k, F  # [0,1]


def calc_av_rate(distances, r_0, E_dis, k_f):  # dt, n):
...
def modify_top(oldtop, newtop, breakpair):
    ...
    deleted = 0
    reached_pairs = False
    data_new = data_all[:]  # workaround to get an independent copy with slice
    ...
    for i in range(len(data_all)):
        if (
            breakpair[0] in data_array[i][0:4] and breakpair[1] in data_array[i][0:4]
    ...
    # print ('Deleted bonds, angles and dihedrals. Now starting deleting pairs: ')
    logging.debug("Deleted bonds, angles and dihedrals. Now starting deleting pairs: ")
    pairs_to_be_deleted = []
    for j in range(len(list_of_dihedrals)):
        if (breakpair[0] in list_of_dihedrals[j][0:4]) and (
            breakpair[1] in list_of_dihedrals[j][0:4]
        ):
    ...
    """
    print (len(list_of_dihedrals))
    print (len(list_of_pairs))

    for l in range(len(pairs_to_be_deleted)):
        if pairs_to_be_deleted[l] not in list_of_pairs:
            print (pairs_to_be_deleted[l])
    """
...
def modify_plumedfile(plumedfile, plumedfile_new, distancefile_new, ruptured_bonds):
    ...
    file.close()
    print(
        "Adjusted Plumedfile: Commented out distances "
        + str(broken_distnbr)
        + " corresponding to breakpairs: "
        + str(rupture_pairs)
        + ". \n Will now write distances to: "
        + str(distancefile_new)
    )
    logging.info(
        "Adjusted Plumedfile: Commented out distances "
        + str(broken_distnbr)
...
def do_montecarlo(transition_prob):
    # compares rupture probabilty with random number
    rupture = False
    random.seed()
    t = random.random()  # x in [0.0,1.0)
    if transition_prob >= t:
        rupture = True
...
def do_kinetic_mc(list_of_nbrs_and_atomtypes, list_of_rates):
    ...
    for i in range(
        len(list_of_rates)
    ):  # scales ~ n/2. Replace with fance binary search to have log(n) but should not be much of a difference here.
...
def do_rejection_KMC(list_of_breakpairs, k_0, topfile, filepath_bonds, filepath_edis):
    ...
    print(
        ...
        + str(breakpair)
        + str(atomtypes)
        + " as candidate in RKMC calculated rupture rate using "
        + str((len(distances)))
        + " distances per bond: "
        + str(k)
        + " per ps."
...
def check_if_error_occured(filepath):
    ...
    for i in range(len(data_array)):
        if "error" in data_all[i]:
            error = True
            print(
                "Warning: Simulation seems to have stopped due to an error: "
                + str(data_all[i : i + 3])
            )
            logging.warning(
                "Simulation seems to have stopped due to an error:"
                + str(data_all[i : i + 3])
            )
...
def write_conditions_in_plumedfile(topfile, indexfile, indexgroup, parameterfile):
    # only use once to create index and top file with all necessary entries / conditons
    ...
    for k in range(len(index_array)):
        if found_group and "[ " in index_all[k]:  # stop at next group
            break
    ...
    for j in range(bonds_pos + 2, len(data_all)):  # go through all bonds
        ...
        if len(data_array[j]) > 0:
            ...
            if nbr2 not in relevant_atoms:
                continue
        # leave out stronger C-N bond (chemically not prone to rupture due to extra electron)
        if ("C", "N") in [
...
if __name__ == "__main__":
    write_conditions_in_plumedfile(
        "topol.top", "index.ndx", "[ Backbone_LY2_LY3_L4Y_L5Y_LYX ]", "ffbonded.itp"
    )
