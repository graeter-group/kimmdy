...
| Name | Description |
| --- | --- |
| [backup_if_existing](#kimmdy.utils.backup_if_existing) | Checks whether a file exists and if so, backs it up. |
| [check_gmx_version](#kimmdy.utils.check_gmx_version) | Check for an existing gromacs installation. |
| [get_atominfo_from_plumedid](#kimmdy.utils.get_atominfo_from_plumedid) | For a plumedid, returns the corresponding atom types and nrs. |
...
### backup_if_existing { #kimmdy.utils.backup_if_existing }
...
|--------|------------------------|-----------------------------------------------------|------------|
| `f`    | [Path](`pathlib.Path`) | Path to a file that will be backed up, if existing. | _required_ |
...
| `plumed`   | [dict](`dict`) | Parsed plumed input file                      | _required_ |
| `top`      | [dict](`dict`) | Topology of the molecular system              | _required_ |
...
| Name                      | Type                     | Description                                                                       | Default    |
|---------------------------|--------------------------|-----------------------------------------------------------------------------------|------------|
| `atomtypes`               | [frozenset](`frozenset`) | Two atomtypes as defined in the respective force field                            | _required_ |
| `ffbonded`                | [dict](`dict`)           | Force field ffbonded.itp file parsed through the rtp parser                       | _required_ |
| `lookup_edissoc_atomtype` | [dict](`dict`)           | Parsed file with dissociation energies per bond between two atomtypes or elements | _required_ |
