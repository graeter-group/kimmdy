...
from kimmdy.topology.atomic import Bond, BondType, Dihedral
...
def test_merge_prm_top(arranged_tmp_path, caplog):
    ...
    top_merge = merge_top_slow_growth(top_a=top_a, top_b=top_b, enable_morph_pairs=True)
    ...
    for k, ref in top_merge_ref.atoms.items():
        new = top_merge.atoms[k]
        assert ref == new

    # assert top_merge.bonds == top_merge_ref.bonds
    # more detailed output:
    for k, ref in top_merge_ref.bonds.items():
        new = top_merge.bonds[k]
        assert ref == new
