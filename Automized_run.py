...
def do_production_run(grofile,topfile,mdpfile, indexfile, tprfile,trrfile):
    ...
    command.wait()
    command=sp.Popen('mpirun mdrun_mpi -v -s '+tprfile+ ' -o ' +trrfile,shell=True) #use gmx mdrun if not on cluster  #use mpirun mdrun_mpi on cluster
    command.wait()
...
def energy_min_after_break(oldcpt, newmdp, oldtpr, newtpr, newtop, indexfile, outgro, newtrr):
    ...
    command.wait()
    command=sp.Popen('mpirun mdrun_mpi -v -s '+newtpr+ ' -c '+ outgro + ' -o ' + newtrr,shell=True)  # use mpirun mdrun_mpi on cluster
    command.wait()


def do_plumed_run(grofile,topfile,mdpfile, indexfile, tprfile,trrfile,plumedfile):

    command=sp.Popen('gmx grompp -p '+topfile+' -c '+grofile+' -r '+grofile+' -f '+mdpfile + ' -n ' +indexfile +  ' -o '+tprfile + ' -maxwarn 70' ,shell=True)
    command.wait()
    command=sp.Popen('mpirun mdrun_mpi  -v -s '+tprfile+ ' -o ' +trrfile   + ' -plumed ' + plumedfile,shell=True) #use gmx mdrun if not on cluster  #use mpirun mdrun_mpi on cluster
    command.wait()



def continue_run (oldcpt, newmdp, oldtpr, newtpr, newtop, indexfile, newtrr, plumedfile):
    command = sp.Popen(" gmx grompp -f " + newmdp + " -p " + newtop + ' -n ' + indexfile + " -c "+ oldtpr + " -r "+oldtpr+" -o " + newtpr + " -t " + oldcpt + ' -maxwarn 75', shell = True)
    command.wait()
    command=sp.Popen('mpirun mdrun_mpi -maxh 22.0 -v -s '+newtpr+' -o '+ newtrr + ' -plumed ' + plumedfile,shell=True)  # use mpirun mdrun_mpi on cluster
    command.wait()
...
def rerun_trajectory(trrfile,tprfile,edrfile,topfile,grofile,mdpfile):
    ...
    command=sp.Popen('gmx grompp -p '+topfile+' -c '+grofile+' -f '+mdpfile+' -o '+tprfile,shell=True)  #Warum nochmal?
    command.wait()
    command=sp.Popen('gmx mdrun -rerun '+trrfile+' -s '+tprfile+' -v -e '+edrfile, shell=True)
    command.wait()
