...
for i in range(20, 30):
    ...
    shutil.copytree(
        basedir / "files_md", basedir / str(i), dirs_exist_ok=False, symlinks=True
    )
    AAstring = rng.choice(
        [
            "A",
        ...
        5,
    )  # extra P for Hydroxyproline, Y for DOPA
    print(AAstring, "".join(AAstring))
    sp.run(
        f"pymol -c -d 'fab B{''.join(AAstring)}Z, pep{i}, ss=2;set pdb_use_ter_records, 0;save pep{i}.pdb, pep{i}'",
        shell=True,
        cwd=currdir,
    )
    ...
    for pos in P_pos:
        if rng.choice([0, 1], 1):
            ...
            sp.run(
                f"sed 's/pep_i/pep{i}/g' ProtoHyp_blank.pml | sed 's/pos/{pos+2}/g' > {i}/ProtoHyp_{i}_{pos+2}.pml",
                shell=True,
                cwd=basedir,
            )
    ...
    for pos in Y_pos:
        if rng.choice([0, 1], 1):
            ...
            sp.run(
                f"sed 's/pep_i/pep{i}/g' TyrtoDop_blank.pml | sed 's/pos/{pos+2}/g' > {i}/TyrtoDop_{i}_{pos+2}.pml",
                shell=True,
                cwd=basedir,
            )
