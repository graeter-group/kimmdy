...
def find_bond_param(atomtypes, filepath, own_r0 = False):
    ...
    if own_r0:
        ...
        logging.debug('Used own r0 from run without force instead of value from itp-file as equilibrium distance.')
        comb1_equil = (atomtypes[0], atomtypes[1])
        comb2_equil = (atomtypes[1], atomtypes[0])
        dic_of_equil_r0 = {('N3', 'CT'):'0.1476', ('CT', 'C'):'0.1535', ('C', 'N'):'0.1341', ('N', 'CT'):'0.1483', ('CT', 'C'):'0.1544'}
...
def do_kinetic_mc(list_of_nbrs_and_atomtypes, list_of_rates):
    ...
    t = random.random() #x in [0.0,1.0)
    rate_running_sum = 0
    for i in range(len(list_of_rates)):   #scales ~ n/2. Replace with fance binary search to have log(n) but should not be much of a difference here. 
...
if __name__ == "__main__":
    ...
    write_conditions_in_plumedfile('topol.top', 'index.ndx', '[ Backbone_LY2_LY3_L4Y_L5Y_LYX ]', 'ffbonded.itp')
