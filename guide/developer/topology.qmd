---
title: Hexala topology
mermaid-format: svg
---

# Hexala

```{python}
from kimmdy.parsing import read_top
from kimmdy.topology.topology import Topology
from pathlib import Path
from kimmdy.misc_helper import top_to_graph
from kimmdy.topology.utils import get_protein_section
```


```{python}
#| eval: true
ala_top = read_top(Path('../../tests/test_files/test_topology/hexala.top'))
top = Topology(ala_top)

ls = top_to_graph(top) 
with open("ala-top.dot", "w") as f:
  f.writelines(ls)
```


```{dot}
//| echo: false
//| label: fig-ala
//| fig-cap: "A diagram of the ala topology"
//| file: ala-top.dot
```


```{python}
#| eval: false
ala_top = read_top(Path('tests/test_files/test_topology/hexala.top'))
top = Topology(ala_top)
```

```{python}
#| eval: false
breakpair = ("9", "15")

# top.break_bond(breakpair)

dhs = get_protein_section(top.top, "dihedrals")
assert dhs
for l in dhs:
  if all([breakpair[0] in l[:4], breakpair[1] in l[:4]]):
    print(l)

# top

top.moleculetypes["Protein"].atomics.keys()
top.moleculetypes["Protein"].__dict__.keys()

```
