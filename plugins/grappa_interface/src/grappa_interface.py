...
def apply_parameters(top: Topology, parameters: dict) -> Topology:
        ...
        for bond_ids, bond_prm in parameters["bonds"].items():
            if not top.bonds.get(bond_ids):
                    logging.warning(f"Ignored parameters with invalid ids: {bond_ids} for bonds")
                    continue
        ...
        for angle_ids, angle_prm in parameters["angles"].items():
            if not top.angles.get(angle_ids):
                logging.warning(f"Ignored parameters with invalid ids: {angle_ids} for angles")
                continue
        ...
        for dihedral_ids, dihedral in parameters["proper_dihedrals"].items():
            if not top.proper_dihedrals.get(dihedral_ids):
                logging.warning(f"Ignored parameters with invalid ids: {dihedral_ids} for proper dihedrals")
                continue
            dihedral_dict = {}
        ...
        for dihedral_ids, dihedral_prm in parameters["improper_dihedrals"].items():
            if not top.improper_dihedrals.get(dihedral_ids):
                logging.warning(f"Ignored parameters with invalid ids: {dihedral_ids} for improper dihedrals")
                continue
            # dihedral_prm order should be [phase, kd]
...
class GrappaInterface(Parameterizer):
    def parameterize_topology(
        ...
        atoms, bonds, radicals = generate_input(current_topology)
        ...
        assert list(parameters.keys()) == ['atoms','bonds','angles','proper_dihedrals','improper_dihedrals'], f"GrAPPa returned unexpected parameters with sections {list(parameters.keys())}."
